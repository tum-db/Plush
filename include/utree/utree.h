/**
 * uTree, published by Youmin Chen, Youyou Lu, Kedong Fang, Qing Wang, and Jiwu Shu, at VLDB[1].
 * Taken and slightly adapted from here: https://github.com/thustorage/nvm-datastructure
 *
 * [1]: Youmin Chen, Youyou Lu, Kedong Fang, Qing Wang, Jiwu Shu. uTree: a Persistent B+-Tree with Low Tail Latency.
...
class btree{
  ...
  public:
    ...
    void printAll();
    bool insert(entry_key_t, char*); // Insert
    bool remove(entry_key_t);        // Remove
    bool search(entry_key_t, char*);       // Search

    std::atomic<long> total_size = 0;
...
void openPmemobjPool() {
  printf("use pmdk!\n");
  char pathname[100] = "/mnt/pmem0/vogel/utree";
...
bool btree::search(entry_key_t key, char* value) {
    const char invalid_value{0ul};
    bool f = false;
    char *prev;
